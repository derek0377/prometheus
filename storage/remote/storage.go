...
func (s *Storage) Send(ret []byte) {

	//fmt.Printf("*****send*****%v\n",string(ret))
	//u, err := url.Parse("http://10.4.4.35:9000/write")
	if s.Url == nil {
		return
	}
	u := s.Url
	...
	if resp.StatusCode/100 != 2 {
		...
		if _, err := ioutil.ReadAll(resp.Body); err != nil {
			fmt.Printf("****http request**result code*%v\n",err)
			return
		} else {
			fmt.Printf("****http request**not 200*\n")
		}
	}  else {
		//fmt.Printf("****http request** 200*\n")
	}
}

func  (s *Storage) Test(t chan bool,i int) {

	c := 0
	t1 := time.Tick(time.Second*5)
	...
	for {
		...
		case  a := <- s.data:
			...
			for _,t := range *a {
				gz.Write( t.Result )
				gz.Write([]byte(  fmt.Sprintf(" value=%0.7f %d000000\n", t.V, t.T)  ))
			}
			c = c + len(*a)
...
func NewStorage(l log.Logger, stCallback startTimeCallback) *Storage {
	...
	//return &Storage{logger: l, localStartTimeCallback: stCallback,cc:0, ll: new([]*TTT) }
	a := &Storage{logger: l, localStartTimeCallback: stCallback,cc:0, data: make(chan *[] * storage.TTTT )}
	a.stop_test = []chan bool{}
	for i:=0;i<40; i++ {
		t := make(chan bool)
		a.stop_test = append(a.stop_test, t)
		go a.Test(t,i)
	}
	return a
}
