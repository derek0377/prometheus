...
var (
	...
	labFilter = map[string] int {
		"label_app" : 0,
		"label_cluster": 0,
		"label_replication_controller":0,
		"label_redis":0,
	}
	...
	MetaTotal = map[string] []byte {}
	MetaTime  = map[string]  time.Time {}
	Lock  = new(sync.Mutex)
)

func SetMeta(key string, v []byte) {

	Lock.Lock()  
    defer Lock.Unlock()

	fmt.Printf("******set labels****%v****%v\n", string(key), string( v ) )
	MetaTotal[key] = v
	MetaTime[key] = time.Now()
}

func GetMeta(key string) []byte {

	Lock.Lock()  
    defer Lock.Unlock()

	data,ok := MetaTotal[key]
	if ok {
		return data
	} else {
		return []byte{}
...
func washOld() {
	...
	now := time.Now()
	de := time.Duration(4* time.Hour)
	for k,v  := range MetaTime {
		if now.Sub(v) > de {
			delete( MetaTotal, k )
			delete( MetaTime, k)
		}
...
func (s *targetScraper) Tag() bool {
    ...
    if strings.Contains(a,"proxy/metrics/cadvisor") == true {
            return true
    }

    if strings.Contains(a, ":9187/metrics") == true {
            return true
    }       
...
func (s *targetScraper) FetchUrl() string {
	return s.URL().String()
}

func (s *targetScraper) Meta() bool {
    a := s.FetchUrl()
    if a == "" {
        return false
    }

    if strings.Contains(a, ":8080") == true {
        return true
    }     
...
func (c *cacheEntry) FetchSs() ([]byte,bool) {


	if c.Suffix == false {
		suff :=  GetMeta(c.PodName)
		if len(suff) == 0 {
			//fmt.Printf("***FetchSs*1**%v\n", string(c.Filter))
...
func (c *scrapeCache) addRef(met string, ref uint64, lset labels.Labels, hash uint64, fin []byte,tagFlag bool,Tflag bool, pod string, filter []byte) {

	if ref == 0 {
		return
	}
	if Tflag == true {
		c.entries[met] = &cacheEntry{ref: ref, lastIter: c.iter, lset: lset, hash: hash, Ss: fin, 
			tagFlag: tagFlag, Tflag: Tflag, Suffix: false, PodName: pod, Filter: filter}
	} else {
		c.entries[met] = &cacheEntry{ref: ref, lastIter: c.iter, lset: lset, hash: hash, Ss: fin, 
			tagFlag: tagFlag, Tflag: Tflag, Suffix: true, PodName: pod, Filter: filter}
	}
...
func (sl *scrapeLoop) run(interval, timeout time.Duration, errc chan<- error) {
	...
	tagFlag := sl.scraper.Tag()

	metaFlag := sl.scraper.Meta()
...
mainLoop:
	for {
		...
		total, added, appErr := sl.append(b, start,tagFlag,metaFlag)
		if appErr != nil {
			level.Warn(sl.l).Log("msg", "append failed", "err", appErr)
			// The append failed, probably due to a parse error or sample limit.
			// Call sl.append again with an empty scrape to trigger stale markers.
			if _, _, err := sl.append([]byte{}, start,tagFlag,metaFlag); err != nil {
		...
		sl.report(start, time.Since(start), total, added, scrapeErr,tagFlag)
		last = start
...
func (sl *scrapeLoop) endOfRunStaleness(last time.Time, ticker *time.Ticker, interval time.Duration) {
	...
	if _, _, err := sl.append([]byte{}, staleTime,false,false); err != nil {
...
func (sl *scrapeLoop) append(b []byte, ts time.Time, tagFlag bool,metaFlag bool) (total, added int, err error) {
	var (
		app            = sl.appender()
		p              = textparse.New(b)
...
loop:
	for p.Next() {
		...
		//fmt.Printf("*****get data******%v**%v\n", yoloString(met), ok)
		if ok {

			ss,res := ce.FetchSs()
			if res == true {
				//fmt.Printf("*****FetchSs******%v\n",string(ss))
				a = append(a, &storage.TTTT{Result:ss, T:t,V:v})
			} else {
				//fmt.Printf("filter******%v\n",yoloString(met))
			}
		...
		if !ok {
			...
			for ; c < total; c ++ {
				...
				zbuf.Write( []byte(v.Name)  )
				zbuf.Write( []byte("=") )
				zbuf.Write( []byte(v.Value)  )
				zbuf.Write( []byte(",") )
			...
			for ; c < total; c ++ {
				...
				if tagFlag == true {
					if v.Name == "pod_name" {
						podName = string( v.Value )
					}

					_,ok := tagFilter[v.Name]
					if ok == true {
						tt.Write( []byte(v.Name)  )
						tt.Write( []byte("=") )
						tt.Write( []byte(v.Value)  )
						tt.Write( []byte(",") )
						cc = cc + 1
					}
				}
			...
			Tflag := false
			newTag := []byte{}
			if tagFlag == true && cc != 0 && podName != "" && podName != "POD"{
				newTag = tt.Bytes()
				newTag = newTag[0: len(newTag)-1]
				a = append(a, &storage.TTTT{Result:newTag, T:t,V:v})
				Tflag = true
			}
			...
			if tagFlag == false {		
				a = append(a, &storage.TTTT{Result:bb, T:t,V:v})
			}

			/*** deal with meta***/
			if metaFlag == true {
				if string( meas.Value ) == "kube_pod_labels" {
					...
					for k:=1; k < total; k ++ {
						...
						_,ok := labFilter[name]
						if ok {
						//if strings.Contains(name,"label_") {

							suffix.Write( []byte(v.Name)  )
							suffix.Write( []byte("=") )
							suffix.Write( []byte(v.Value)  )
							suffix.Write( []byte(",") )
						}
					...
					suff := suffix.Bytes()
					if len(suff) > 1 {
						suff = suff[0:len(suff) - 1]
					}
					SetMeta(podName, suff)
				}
			}
			...
			//sl.cache.addRef(mets, ref, lset, hash,bb)
			sl.cache.addRef(mets, ref, lset, hash,bb,tagFlag,Tflag, podName,newTag)
		}
		added++
	}
	app.AddFasts(&a)
...
func (sl *scrapeLoop) report(start time.Time, duration time.Duration, scraped, appended int, err error,tagFlag bool) error {
	...
	app := sl.appender()
	//just support up
	if err := sl.addReportSampleX(app, scrapeHealthMetricName, ts, health,tagFlag); err != nil {
		app.Rollback()
		return err
	}
...
func (sl *scrapeLoop) addReportSample(app storage.Appender, s string, t int64, v float64) error {
	...
	switch err {
	case nil:
		sl.cache.addRef(s, ref, lset, hash,[]byte{},false,false,"",[]byte{})
		return nil
	case storage.ErrOutOfOrderSample, storage.ErrDuplicateSampleForTimestamp:
		return nil
	default:
...
func (sl *scrapeLoop) addReportSampleX(app storage.Appender, s string, t int64, v float64,tagFlag bool) error {
	ce, ok := sl.cache.get(s)
	if ok {
		a := []*storage.TTTT{ &storage.TTTT{Result:ce.Ss, T:t,V:v}  }
		app.AddFasts(&a)
	...
	switch err {
	case nil:
		sl.cache.addRef(s, ref, lset, hash,bb,false,false,"",[]byte{})
		return nil
	case storage.ErrOutOfOrderSample, storage.ErrDuplicateSampleForTimestamp:
		return nil
	default:
